...
class AxialBottleneck(nn.Module):
    def __init__(self, dim, heads, image_size):
        super().__init__()
        self.ax = AxialAttention(
            dim=dim,
            dim_index=1,
            axial_pos_emb_shape=(image_size, image_size),
            heads=heads,
        )
...
class DecoderBlock(nn.Module):
    def __init__(
        self, in_channels, skip_channels, out_channels, size, depth=1, heads=8
    ):
        super().__init__()
        ...
        self.ax = nn.ModuleList(
            [
                AxialBottleneck(
                    dim=in_channels + skip_channels, heads=heads, image_size=size
                )
...
class TransInUnet(nn.Module):
    def __init__(self, image_size, nc, init_dim=16):
        ...
        self.up4 = UnetDecoderBlock(
            init_dim * 2, init_dim, init_dim, depth=1, heads=1, size=image_size
        )
