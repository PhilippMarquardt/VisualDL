...
def predict_mlp(model, data, device, mlp_output_type, range_predictions):
    ...
    for dt in data:
        ...
        with torch.cuda.amp.autocast():
            ...
            if (
                mlp_output_type == "pairwise"
            ):  # pairwise: e.g. x and y values next to each other in output layer
                prediction = np.split(prediction, prediction.size / 2)
            elif (
                mlp_output_type == "ranges"
            ):  # e.g. predict range of x values in addition to y values
                range_pred_output = []
...
class MLPTrainer:
    def __init__(self, cfg_path: str):
        ...
        assert (
            self.cfg["type"] == "mlp"
        ), "Provided yaml file must be a regression config!"
    ...
    def train(self):
        ...
        model = get_mlp_model(
            self.cfg["settings"]["in_features"], self.cfg["settings"]["out_features"]
        )
        ...
        for i in range(epochs):
            ...
            model.train()
            for cnt, (x_pred, y_pred) in enumerate(
                training_bar
            ):  # what ist output of dataloader?
                ...
                training_bar.set_description(
                    "Training: Epoch:%i, Loss: %.3f, best: %.2f"
                    % (i, float(loss_average / (cnt + 1)), best_valid_loss)
                )
                    ...
                    torch.save(
                        {
                            ...
                            "range_predictions": self.cfg["settings"][
                                "range_predictions"
                            ],
