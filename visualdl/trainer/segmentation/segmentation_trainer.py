...
class SegmentationTrainer(TrainerBase):
    def __init__(self, cfg_path: dict):
        ...
        assert (
            self.cfg["type"] == "segmentation"
        ), "Provided yaml file must be a segmentation model config!"
        assert len(self.cfg["settings"]["batch_size"]) == len(self.cfg["models"])
        assert len(self.weights) == len(self.cfg["models"])
        logging.info(
            f"Training segmentation model with the following config:\n {pprint.pformat(self.cfg)}"
        )
        self.add_object_detection_model = self.cfg["settings"][
            "add_object_detection_model"
        ]
        self.use_attention = self.cfg["settings"]["use_attention"]
        logging.info("Creating segmentation models...")
        self.models = [
            SegmentationModel(
                models["backbone"],
                models["decoder"],
                ...
                self.use_attention,
                self.custom_data,
                self.cfg["settings"]["calculate_distance_maps"],
            )
            for models, weight in zip(self.cfg["models"], self.weights)
        ]
        logging.info("Finished creating segmentation models")

    def train(self):
        logging.info("Starting training training!")
        for cnt, (train_loader, valid_loader, test_loader, model) in enumerate(
            zip(self.train_loaders, self.valid_loaders, self.test_laoders, self.models)
        ):
            logging.info(f"Training {model.name}")
            model.train(
                train_loader, valid_loader, test_loader, self.cfg["settings"]["epochs"]
            )
