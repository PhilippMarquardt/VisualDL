...
def create_files(folder, nc=1, single_class=False):
    ...
    for cnt, (key, val) in enumerate(files.items()):
        with open(
            os.path.join(
                ...
                key.replace(".png", ".txt").replace(".jpg", ".txt"),
            ),
            "w",
        ) as handle:
            for va in val:
    ...
    for name in no_anno:
        with open(
            os.path.join(
                os.path.join(folder, "labels"),
                name.replace(".png", ".txt").replace(".jpg", ".txt"),
            ),
            "w",
        ) as handle:
            handle.write("")
...
class DetectionTrainer:
    def __init__(self, cfg_path: dict):
        ...
        datafile = os.path.join(
            self.this_dir, r"../../dependencies/yolov5/coco128.yaml"
        )
        ...
        modelfile = os.path.join(
            self.this_dir,
            f"../../dependencies/yolov5/models/yolov5{self.cfg['settings']['modelsize']}.yaml",
        )
    ...
    def train(self):
        ...
        hyperfile = os.path.join(
            self.this_dir, r"../../dependencies/yolov5/hyp.scratch.yaml"
        )
        # opts = parse_opt()
        ...
        if "device" in self.cfg["settings"]:
            run(
                ...
                imgsz=self.cfg["settings"]["imgsize"],
                epochs=self.cfg["settings"]["epochs"],
                batch_size=self.cfg["settings"]["batch_size"],
                batch=self.cfg["settings"]["batch_size"],
                data=datafile,
                cfg=modelfile,
                workers=0,
                hyp=hyperfile,
                project=self.cfg["data"]["save_folder"],
                device="cpu",
                weights=self.cfg["data"]["weights"],
            )
        else:
            run(
                custom_data=self.cfg["settings"]["custom_data"],
