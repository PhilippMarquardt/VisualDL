...
class VideoTrainer:
    def __init__(self, cfg_path: dict):
        ...
        self.encoder = timm.create_model(
            self.cfg["settings"]["modelname"],
            pretrained=True if is_internet_connection_availible() else False,
            features_only=True,
        )
    ...
    def train(self):
        train_x = torch.rand(
            (200, 10, 3, 128, 128), dtype=torch.float
        )  # 200 videos, 10 bilder pro video, 3 channel, 128 width, 128 height
