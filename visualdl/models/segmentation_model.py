...
class SegmentationModel(ModelBase):
    def __init__(
        ...
        if decoder_name.lower() == "U2NET".lower():
            ...
            modelstring = (
                f"MedT(img_size = {max_image_size}, imgchan = {3}, num_classes = {nc})"
            )
        elif decoder_name.lower() == "unetr":
            model = UNETR(
                in_channels=in_channels,
                out_channels=nc,
                img_size=(max_image_size, max_image_size),
            )
            ...
            if use_attention:
                model = eval(
                    f'smp.create_model(arch="{decoder_name}", encoder_name="{encoder_name}", encoder_weights={encoder_weights}, in_channels={in_channels}, classes = {nc}, image_size = {max_image_size}, decoder_attention_type = "{"scse"}")'
                )
        ...
        super().__init__(
            ...
            class_weights,
            model=model,
            calculate_weight_map=calculate_weight_map,
            weight=weight,
            save_folder=save_folder,
            early_stopping=early_stopping,
            custom_data=custom_data,
            calculate_distance_maps=calculate_distance_maps,
        )
    ...
    def train(
        ...
        train_all_epochs(
            ...
            writer=self.writer,
            name=f"{self.encoder_name}, {self.decoder_name}",
