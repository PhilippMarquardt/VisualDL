...
class AxialBlock_gated_data(nn.Module):
    ...
    def __init__(
        ...
        self.conv_down = conv1x1(inplanes, width)
        self.bn1 = norm_layer(width)
        self.hight_block = AxialAttention_gated_data(
            width, width, groups=groups, kernel_size=kernel_size
        )
        self.width_block = AxialAttention_gated_data(
            width,
            width,
            groups=groups,
            kernel_size=kernel_size,
...
class mix(nn.Module):
    def __init__(
        ...
        self.layer1_p = self._make_layer(
            block, int(128 * s), layers[0], kernel_size=(img_size_p // 2)
        )
    ...
    def _forward_impl(self, x):
        ...
        # x = torch.add(x, x3)
        x = F.relu(
            F.interpolate(self.decoder3(x3), scale_factor=(2, 2), mode="bilinear")
        )
        x = torch.add(x, x2)
        x = F.relu(
...
class ResAxialAttentionUNetshallow(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x = torch.add(x, x1)
        x = F.relu(
            F.interpolate(self.decoder5(x1), scale_factor=(2, 2), mode="bilinear")
        )
        x = self.soft(self.adjust(F.relu(x)))
        # pdb.set_trace()
        return x
...
def resxialunet128s(pretrained=False, **kwargs):
    model = ResAxialAttentionUNet(
        AxialBlock_dynamic, [1, 2, 4, 1], s=0.125, img_size=128, imgchan=1, **kwargs
    )
    return model
...
def resaxialunet_dyn(pretrained=False, **kwargs):
    model = ResAxialAttentionUNet(
        AxialBlock_dynamic, [1, 2, 4, 1], s=0.125, img_size=512, imgchan=3, **kwargs
    )
    return model
...
def resxialunet_wopos(pretrained=False, **kwargs):
    model = ResAxialAttentionUNet(
        AxialBlock_wopos, [1, 2, 4, 1], s=0.125, img_size=128, imgchan=3, **kwargs
    )
    return model
...
def resunet(pretrained=False, **kwargs):
    model = ResAxialAttentionUNet(
        AxialBlockmod, [1, 2, 4, 1], s=0.125, img_size=128, imgchan=1, **kwargs
    )
    return model
...
def unetplusplus(pretrained=False, **kwargs):
    model = unetplus(
        AxialBlockmod, [1, 2, 4, 1], s=0.125, img_size=128, imgchan=3, **kwargs
    )
    return model
...
def mix_net(pretrained=False, **kwargs):
    model = mix(
        AxialBlock_dynamic, [1, 2, 4, 1], s=0.125, img_size=128, imgchan=3, **kwargs
    )
    return model
...
def mix_net_512(pretrained=False, **kwargs):
    model = mix_512(
        AxialBlock, [1, 2, 4, 1], s=0.125, img_size=512, imgchan=3, **kwargs
    )
    return model
...
def mix_net_gated_d(pretrained=False, **kwargs):
    model = mix(
        AxialBlock_gated_data, [1, 2, 4, 1], s=0.125, img_size=128, imgchan=1, **kwargs
    )
    return model
...
def resxialunet128s_shallow(pretrained=False, **kwargs):
    model = ResAxialAttentionUNetshallow(
        AxialBlockmod, [1, 2, 4, 1], s=0.125, img_size=128, imgchan=3, **kwargs
    )
    return model
