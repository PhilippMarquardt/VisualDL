...
class TrainerBase(ABC):
    def __init__(self, cfg):
        ...
        assert (
            len(self.cfg["settings"]["metrics"]) > 0
        ), "You must provide atleast one metric"
        assert all(
            [
                self.cfg["settings"]["gradient_accumulation"] % batch == 0
                for batch in self.cfg["settings"]["batch_size"]
            ]
        )
        ...
        self.metrics = [
            eval(f"{metric['name']} ({metric['params']})")
            for metric in self.cfg["settings"]["metrics"]
        ]
        self.monitor_metric = eval(
            f"{self.cfg['settings']['monitor_metric_name']} ({self.cfg['settings']['monitor_metric_params']})"
        )
        ...
        if self.valid_path != "":
            self.valid_loaders = [
                get_dataloader(
                    dataset(self.valid_path, transform=valid_trans), batch_size, 0
                )
                for batch_size in self.batch_sizes
            ]
        if self.test_path != "":
            self.test_loaders = [
                get_dataloader(
                    dataset(self.test_path, transform=valid_trans), batch_size, 0
                )
                for batch_size in self.batch_sizes
            ]
        self.class_weights = (
            torch.FloatTensor(self.train_loaders[0].dataset.class_weights).to(
