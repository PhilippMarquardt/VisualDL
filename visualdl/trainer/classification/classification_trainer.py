...
class ClassificationTrainer(TrainerBase):
    def __init__(self, cfg_path: dict):
        ...
        assert (
            self.cfg["type"] == "classification"
        ), "Provided yaml file must be a classification model config!"
        assert len(self.cfg["settings"]["batch_size"]) == len(self.cfg["model_names"])
        logging.info(
            f"Training classification model with the following config:\n {pprint.pformat(self.cfg)}"
        )
        self.models = [
            ClassificationModel(
                name,
            ...
            for name, weight in zip(self.cfg["model_names"], self.weights)
        ]
